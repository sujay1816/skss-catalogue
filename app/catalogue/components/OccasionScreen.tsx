...
// ─── OccasionScreen ──────────────────────────────────────────────────────────
// Shows only occasions from the admin panel — no hardcoded fallbacks.
// If none are configured in the DB, we skip straight to browsing.
// Shows up to 6 in a responsive 2-column grid (3 rows max).
export function OccasionScreen({
  ...
  // If no occasions configured in admin, go straight to browsing
  ...
  return (
    ...
    >
      ...
      {/* Grid — only admin occasions, responsive 2 or 3 col */}
      <div
        style={{
          display: 'grid',
        ...
        {items.map(occ => (
          ...
          >
            {occ.image_url ? (
              ...
              <div style={{ width: '100%', height: '100%', background: 'linear-gradient(145deg, #F5EDE3, #EDE0D0)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 38 }}>
                🥻
              </div>
            )}
