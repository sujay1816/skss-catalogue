...
// - "Buy Now" button REMOVED (UX-I / product requirement — no direct buy)
// - Footer replaced with "Book a Call on WhatsApp" as sole CTA + heart
// - UX-D: CTA label unified to "Book a Call on WhatsApp"
// - B-3: onPointerCancel added to image swipe to clear stuck flag
// - UX-C: tapping similar saree cards now calls onSelectSimilar (already worked), shown more prominently
// - UX-G: "You may also like" falls back to all products if category/fabric match returns 0
export function DetailSheet({
  product, isLoved, onClose, onLove, waNum, flashSale, onBookCall, allProducts, onSelectSimilar,
}: {
  ...
  const imgSwipe   = useRef({ on: false, x0: 0 })  // B-3
  ...
  // UX-G: widen similar fallback so it's rarely empty
  const similar = (() => {
  ...
  // B-3: image swipe with onPointerCancel to prevent stuck flag
  ...
  const imgCancel = () => { imgSwipe.current.on = false }  // B-3
      ...
      <div ref={sheetRef} style={{ position: 'fixed', bottom: 0, left: '50%', transform: 'translateX(-50%)', width: '100%', maxWidth: 480, maxHeight: '92dvh', zIndex: 301, background: '#0f0a06', borderRadius: '20px 20px 0 0', display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: '0 -16px 60px rgba(0,0,0,0.95)', animation: 'sheetUp 0.38s cubic-bezier(0.32,0.72,0,1)' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          ...
          <div style={{ padding: '16px 20px 36px' }}>
            ...
            {sortedVars.length > 0 && (
              <div style={{ marginBottom: 22 }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                  {sortedVars.map(v => (
                    <div key={v.id} style={{ display: 'flex', alignItems: 'center', gap: 8, background: v.stock === 0 ? 'rgba(255,255,255,0.02)' : 'rgba(255,255,255,0.05)', border: v.stock === 0 ? '1px solid rgba(255,255,255,0.05)' : '1px solid rgba(255,255,255,0.09)', borderRadius: 24, padding: '6px 14px 6px 8px', opacity: v.stock === 0 ? 0.45 : 1 }}>
            ...
            {/* UX-G: improved similar section — always shows something */}
            {similar.length > 0 && (
        ...
        {/* Footer — UX-D: single CTA "Book a Call on WhatsApp", no "Buy Now" */}
        <div style={{ padding: '12px 16px 32px', borderTop: '1px solid rgba(255,255,255,0.08)', display: 'flex', gap: 10, flexShrink: 0, background: 'rgba(10,6,2,0.98)' }}>
          ...
          {/* UX-D + UX-I: replaced "Buy Now" with single WhatsApp CTA */}
