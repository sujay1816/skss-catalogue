...
// B-1 FIX: onSubmit is now awaited in try/catch; loading state resolves even on error;
// onClose is called only after submit succeeds, not immediately.
export function PhoneCaptureSheet({
  wishlist, waNum, onClose, occasion, onSubmit,
}: {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true)
    try {
      onSubmit(n, p)   // fires WA + saves to Supabase
      onClose()        // only close after submit succeeds
    } catch {
      setLoading(false)
        ...
        <div style={{ padding: '16px 24px 0' }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 6 }}>
            <div>
              ...
              <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.4)', marginTop: 4, lineHeight: 1.5 }}>Just your name and number so we know who to expect on WhatsApp.</p>
            </div>
