...
// UX-J: small heart on card to save directly without opening detail
// UX-F: isLoved prop to show already-saved indicator on card
// UX-A: LIKED/NOPE stamps now flash on button-swipe too (via showStamp prop)
// B-3: onPointerCancel on image prevents stuck imgSwipe flag
export function TinderCard({
  ...
  wasSeen: boolean
  isFirstCard: boolean
  isLoved: boolean       // UX-F
  onToggleSave: () => void  // UX-J
}) {
  ...
  const threshold = cardW * 0.25   // SW-4

  // SW-5: back-card eased response
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      {/* UX-J: heart button on card + UX-F: filled if already saved */}
      {isTop && (
        <button
          ...
          {sortedVars.length > 0 && (
            <div style={{ display: 'flex', gap: 5, alignItems: 'center' }}>
              {sortedVars.slice(0, 5).map(v => (
                <div key={v.id} style={{ width: 14, height: 14, borderRadius: '50%', background: v.colourHex || '#8B1A2B', border: '2px solid rgba(255,255,255,0.5)', boxShadow: '0 1px 4px rgba(0,0,0,0.6)', opacity: v.stock === 0 ? 0.3 : 1, position: 'relative' }}>
